...
export class StealthService {
  ...
  async humanTyping(page, selector, text, options = {}) {
    logger.debug(`Human typing: ${text.substring(0, 20)}...`);

    if (!options.directType && options.delay !== 0) {
      await this.applyDelay({ action: 'typing' });
    }

    if (!options.directType && selector && options.clear) {
      try {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      const charDelay = this._naturalRandom(this.config.minTypingDelay, this.config.maxTypingDelay);
