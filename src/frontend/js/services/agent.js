    ...
    async runAgenticLoop(goal, options = {}) {
        ...
        try {
            while (step < maxSteps) {
                ...
                const imageDataUrl = typeof resizedScreenshot === 'string' 
                    ? resizedScreenshot 
...
Previous Actions:
${history.slice(-5).map(h => `- ${h.action}: ${h.reason}`).join('\n')}
...
- For login forms, use credentials from user profile`;
                ...
                try {
                    ...
                    logger.error("AI Reasoning failed:", typeof aiErr === 'object' ? JSON.stringify(aiErr, null, 2) : aiErr);
                    const errorMessage = aiErr.message || JSON.stringify(aiErr);
                    if (step === 1) {
                        decision = { action: 'WAIT', reason: `AI initialization - ${errorMessage}`, value: '2000' };
                    } else {
                ...
                try {
                    ...
                    switch (decision.action) {
                        case 'NAVIGATE':
                            await currentPage.goto(decision.target, { waitUntil: 'domcontentloaded', timeout: 30000 });
                            break;
                        case 'CLICK':
                        ...
                        case 'SCROLL':
                            ...
                            await this.stealth.humanScroll(currentPage, decision.value || 500, decision.value || 'down');
                            break;
                        case 'WAIT':
     ...
     * Enhanced to prioritize session check and support YOLO Mode (auto-creation/actions)
     */
    async universalLogin(targetUrl, credentials, config = {}) {
        logger.info(`Starting Universal Login for ${targetUrl}...`);
        
        // 1. Check if we're already logged in (using session restoration)
        // Navigate and check for "Sign In" vs "Account/Profile" indicators
        const { page } = await this.ensureBrowser({ headless: false, useSession: true });
        
        // Quick check navigation
        logger.info(`Checking existing session state for ${targetUrl}...`);
        try {
            await page.goto(targetUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
            await page.waitForTimeout(3000);
            
            // Vision check for login status
            const screenshot = await page.screenshot({ encoding: 'base64' });
            const resized = await this.imageResize.resize(screenshot);
            const imageDataUrl = typeof resized === 'string' ? resized : `data:image/png;base64,${resized}`;
            ...
            const checkResp = await this.puter.ai.chat(
                sessionCheckPrompt, 
                imageDataUrl, 
        ...
        // 2. Proceed with full login flow if not logged in
        ...
        return await this.runAgenticLoop(
            `Navigate to ${targetUrl} and log in.
            
            Credentials:
            Username/Email: ${credentials.username}
            Password: ${credentials.password}
            
            Universal Login Protocol (Vision-First):
            1. Analyze screenshot for Login/Sign-In fields.
            2. Enter credentials using visual cues (labels, icons).
            3. Prioritize Social Login (Google/GitHub) if available and matches credentials.
            4. ${yoloModeInstruction}
            5. Do NOT rely on HTML structure. Use visual elements only.
            
            Return FINISH when successfully logged in (dashboard visible).`,
            {
                headless: false,
                maxSteps: 40,
                keepBrowserOpen: true
            ...
            const popupPromise = context.waitForEvent('page').catch(e => null);
             ...
             try {
                ...
                try {
                    ...
                    const googleFrame = frames.find(f => f.url().includes('accounts.google.com/gsi/button'));
                    if (googleFrame) {
    ...
    async solveTuringQuizzes() {
        ...
        try {
            ...
            await page.goto('https://developers.turing.com/dashboard/home', { waitUntil: 'domcontentloaded', timeout: 30000 });
            await page.waitForTimeout(3000);
...
- If you see a results screen with a score, use FINISH`;
                ...
                try {
                    switch (decision.action) {
                        ...
                        case 'NAVIGATE':
                            ...
                            const submitButtons = await page.$$('button:has-text("Submit"), button:has-text("Next"), button:has-text("Continue")');
                    ...
                    if (page.url().includes('result') || page.url().includes('complete') || page.url().includes('score')) {
    ...
    async autoApplyToJobs(criteria) {
        ...
        try {
            ...
            const jobsToApply = criteria.maxApplications
                ? jobs.slice(0, criteria.maxApplications)
                : jobs;
            ...
            for (const job of jobsToApply) {
                ...
                try {
                    ...
                    const tailorResult = await this.autoApplyService.tailorCV(
                        job,
                        criteria.userCV
                    );
...
7. Return FINISH only when you see a 'Success', 'Application Submitted', or similar confirmation screen.`,
    ...
    async _mockAIDecision(goal, step, page) {
        if (goal.includes('Turing')) {
            if (step === 1) return { action: 'NAVIGATE', target: 'https://www.turing.com/login', reason: 'Navigating to Login' };
            if (step === 2) return { action: 'TYPE', target: '#email', value: 'user@example.com', reason: 'Entering credentials' };
            // ... more steps
        }
