  ...
  getImageSize(dataUrl) {
    ...
    async resizeWithMetadata(imageData, options = {}) {
        ...
        try {
            ...
            return {
                image: typeof imageData === 'string' && imageData.startsWith('data:') ? imageData : `data:image/png;base64,${imageData}`,
      ...
      // Calculate resulting dimensions manually if we use 'inside'
      let resultWidth = metadata.width;
      let resultHeight = metadata.height;
      ...
      logger.error('Image resize with metadata failed:', error.message);
      // Fallback: assume no resizing occurred
      return {
        image:
          typeof imageData === 'string' && imageData.startsWith('data:')
            ? imageData
            : `data:image/png;base64,${imageData}`,
        metadata: { width: 1280, height: 720 }, // Default viewport fallback
      };
    }
  }
}
